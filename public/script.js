...
magnetActive = false;
let pendingBoosts = { slow: false, magnet: false }; // boost comprati prima della partita
let particleArray = [];
...
// Se ci sono boost pendenti, attivali subito
...
const link = `https://shadow-challenge.onrender.com`;
...
if(data.url) window.location.href=data.url; // vai a Stripe Checkout
}catch(err){ console.error("Errore boost:",err); }
});
});

// ------------------ Controllo ritorno da Stripe ------------------
